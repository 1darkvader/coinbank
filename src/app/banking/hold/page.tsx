        ...
        <div className="max-w-7xl mx-auto px-6 lg:px-8">
          ...
          <div className="grid md:grid-cols-2 lg:grid-cols-3 gap-8 mb-20">
            {[
              ...
              <div key={feature.title} className="banking-card rounded-2xl p-8 hover:scale-105 transition-transform">
          ...
          <div className="banking-card rounded-3xl p-8 mb-20">
            <div className="text-center mb-12">
              <h2 className="text-3xl font-bold text-white mb-4">Institutional-Grade Protection</h2>
              <p className="text-gray-400">Trusted by professionals worldwide</p>
            </div>
          ...
          <div className="grid lg:grid-cols-2 gap-12 mb-20">
            <div>
              ...
              <div className="space-y-6">
                {[
                  ...
                  <div key={layer.title} className="flex items-start space-x-4">
                    <CheckCircle2 className="w-6 h-6 text-emerald-400 mt-1 flex-shrink-0" />
                    <div>
                      <h3 className="font-semibold text-white mb-2">{layer.title}</h3>
            ...
            <div className="banking-card rounded-2xl p-8">
              <h3 className="text-xl font-bold text-white mb-6">Security Dashboard</h3>
              <div className="space-y-4">
                {[
                  { label: 'Wallet Status', status: 'Secure', color: 'text-emerald-400' },
                  { label: 'Multi-Sig Setup', status: '3 of 5 Required', color: 'text-blue-400' },
                  { label: 'Last Security Scan', status: '2 minutes ago', color: 'text-cyan-400' },
                  { label: 'Backup Status', status: 'Up to date', color: 'text-emerald-400' },
                  { label: 'Access Logs', status: 'All verified', color: 'text-emerald-400' }
                ].map((item, index) => (
                  <div key={item.label} className="flex items-center justify-between py-3 border-b border-gray-700">
                    <span className="text-gray-400">{item.label}</span>
                    <span className={`${item.color} font-medium`}>{item.status}</span>
                  </div>
                ))}
              </div>
              <button className="professional-button w-full mt-6 rounded-xl py-3">
          ...
          {/* CTA Section */}
          <div className="text-center bg-gradient-to-r from-blue-500/10 via-cyan-500/10 to-teal-500/10 rounded-3xl p-12 border border-cyan-500/20">
